...
export function PatternTopicPageContent({ category }: PatternTopicPageContentProps) {
  ...
  const sectionIds = subConcepts.map((sub) => ({ id: toAnchorId(sub.name), label: sub.name }));
  ...
  const previousCategory = currentIndex > 0 ? categoryOrder[currentIndex - 1] : null;
  const nextCategory = currentIndex < categoryOrder.length - 1 ? categoryOrder[currentIndex + 1] : null;

  const completionPercent = Math.round((completedSections.size / topic.subConcepts.length) * 100);
    ...
    <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
      ...
      <section className="reveal-up-delay mt-8 grid gap-6 lg:grid-cols-[260px_1fr]">
        <aside className="h-fit rounded-xl border border-slate-200/90 bg-white/70 p-4 backdrop-blur-sm lg:sticky lg:top-6 dark:border-slate-600/70 dark:bg-slate-900/40">
          <h2 className="text-sm font-semibold text-slate-900 dark:text-slate-100">Learning wayfinding</h2>
          <p className="mt-1 text-[11px] text-slate-600 dark:text-slate-300">
            Track completion and jump across sub-concepts while reviewing implementations.
          </p>
          ...
          <div className="mt-3">
            ...
            <p className="mt-1 text-[11px] text-slate-600 dark:text-slate-300">
              {completedSections.size}/{topic.subConcepts.length} sections reviewed ({completionPercent}%)
            </p>
          </div>
          ...
          <ul className="mt-3 space-y-1.5">
            {sectionIds.map((section) => {
              ...
              return (
                <li key={section.id} className="rounded-md border border-slate-200/80 bg-white/80 p-2 dark:border-slate-600/70 dark:bg-slate-900/40">
          ...
          <div className="mt-4 space-y-2 border-t border-slate-200/80 pt-3 dark:border-slate-600/70">
            {previousCategory && (
              <Link href={categoryPaths[previousCategory]} className="block text-[11px] text-slate-600 hover:text-primary dark:text-slate-300">
                Previous: {categoryLabels[previousCategory]}
              </Link>
            )}
            {nextCategory && (
              <Link href={categoryPaths[nextCategory]} className="block text-[11px] text-slate-600 hover:text-primary dark:text-slate-300">
          ...
          <Card className="principle-card">
            ...
            <CardContent className="space-y-5 text-sm text-slate-700">
              ...
              {topic.subConcepts.map((sub) => {
                ...
                return (
                  <section key={sub.demoKey} id={sectionId} className="scroll-mt-24 space-y-3 rounded-lg border border-slate-200/80 bg-white/50 p-3 dark:border-slate-600/70 dark:bg-slate-800/30">
              ...
              <Separator />
              <p>
                <span className="font-semibold text-slate-900">Key takeaway:</span> {topic.keyTakeaway}
              </p>
            </CardContent>
          </Card>
        </div>
